...
local function RemoveItem(src, item, amount)
    ...
    local slot
    if slots then
        if item == Config.Items.Screwdriver then
            slot = slots.screwdriver
            slots.screwdriver = nil
        elseif item == Config.Items.FakePlate then
            slot = slots.fakeplate
            slots.fakeplate = nil
        end
    end
...
AddEventHandler('playerDropped', function()
    local src = source
    ActionCooldown[src] = nil
    ItemUseSlots[src] = nil
end)
